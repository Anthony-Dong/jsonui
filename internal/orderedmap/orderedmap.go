...
func NewWithSize(size int) *OrderedMap {
	o := OrderedMap{}
	o.keys = make([]string, 0, size)
	o.values = make(map[string]interface{}, size)
	o.escapeHTML = true
	o.useNumber = false
	return &o
}
...
func (o *OrderedMap) ForeachErr(foreach func(key string, value interface{}) error) error {
	for _, key := range o.keys {
		if err := foreach(key, o.values[key]); err != nil {
			return err
		}
	}
	return nil
		...
		if delim, ok := token.(json.Delim); ok {
			...
			case '[':
				if values, ok := o.values[key].([]interface{}); ok {
					if err = decodeSlice(dec, values, o.escapeHTML, o.useNumber); err != nil {
						return err
					}
				} else if err = decodeSlice(dec, []interface{}{}, o.escapeHTML, o.useNumber); err != nil {
					return err
				}
			}
		}
...
func decodeSlice(dec *json.Decoder, s []interface{}, escapeHTML, useNumber bool) error {
	for index := 0; ; index++ {
		token, err := dec.Token()
		if err != nil {
			return err
			...
			case '[':
				if index < len(s) {
					if values, ok := s[index].([]interface{}); ok {
						if err = decodeSlice(dec, values, escapeHTML, useNumber); err != nil {
							return err
						}
					} else if err = decodeSlice(dec, []interface{}{}, escapeHTML, useNumber); err != nil {
						return err
					}
				} else if err = decodeSlice(dec, []interface{}{}, escapeHTML, useNumber); err != nil {
					return err
